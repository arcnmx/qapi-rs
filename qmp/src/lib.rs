...
#![doc(html_root_url = "https://docs.rs/qapi-qmp/0.11.0")]
#![allow(deprecated)]

use {
    serde::{Deserialize, Serialize},
    std::{convert::TryFrom, io, string::String as StdString},
};
...
impl<C> TryFrom<QmpMessage<C>> for qapi_spec::Response<C> {
    ...
    fn try_from(m: QmpMessage<C>) -> Result<Self, Self::Error> {
        match m {
            ...
            QmpMessage::Event(..) => Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "QMP event where a response was expected",
            )),
        }
    }
}
...
impl device_add {
    pub fn new<
        D: Into<StdString>,
        I: Into<Option<StdString>>,
        B: Into<Option<StdString>>,
        P: IntoIterator<Item = (StdString, qapi_spec::Any)>,
    >(
        driver: D,
        id: I,
        bus: B,
